customElements.define(
  ...
  class extends HTMLElement {
    constructor() {
      ...
      //this.pathJSON = `https://mainline.i3s.unice.fr/repository.json`;
      this.pluginsJSON = {};
        ...
        this.fetchJSON(this.pathJSON).then(_d => {
          for (const p in _d.plugs) {
            this.fetchJSON(`${_d.plugs[p]}/main.json`).then(_p => {
              ...
              _indexPlugs++;
              if (
                _index == Object.keys(_d.plugs).length &&
                _index == _indexPlugs
              )
                resolve(_pluginsJSON);
            });
            _index++;
          }
        });
    ...
    setOptionsMenuPlugin(_pluginsJSON) {
      ...
      let _elements = ``;
      Object.keys(_pluginsJSON).map((elem, index) => {
        if (elem !== `synth` && elem !== `Mixing`)
          _elements += `<option>${elem}</option>`;
      });
    ...
    selectCategory(e) {
      ...
      let _img = ``;
      Object.keys(_categoryPluginJSON).map((elem, index) => {
        _img = document.createElement(`img`);
        _img.id = `img_${elem}`;
        _img.src = `${_categoryPluginJSON[elem].url}/${_categoryPluginJSON[
          elem
        ].thumbnail.replace(`./`, ``)}`;
        ...
        _img.onclick = e =>
          this.selectPlugin(
            e,
            _categoryPluginJSON[parseInt(e.target.id.replace(`img_`, ``))]
          );
    ...
    buildPlugin(_pluginSettings) {
      var plugin = new window[_pluginSettings.className](
        this.ctx,
        _pluginSettings.baseURL
      );
    ...
    audioConnexion(node) {
      ...
      if (node.type != `amp`) {
        if (
          this.audioPluginConnexion.length > 0 &&
          this.audioPluginConnexion[lastIndex].type != `amp`
        )
      ...
      if (this.audioPluginConnexion.length > 1) {
        for (let i = 0; i < this.audioPluginConnexion.length - 1; i++) {
          console.log(
            `plug1+2:`,
            this.audioPluginConnexion[i],
            this.audioPluginConnexion[i + 1]
          );
          this.audioPluginConnexion[i].connect(
            this.audioPluginConnexion[i + 1]
          );
    ...
    getOrderArray() {
      ...
      for (let i = 0; i < _nbLI; i++) {
        this.pluginsList.querySelectorAll(`li`).forEach((_li, _index) => {
          if (_li.style.order == i)
            _tab.push(parseInt(_li.getAttribute(`data-order`)));
        });
      }
      return _tab;
    }
    ...
    drop(event) {
      ...
      if (event.target.className == `divDroppable`) {
        ...
        // swipe plugins
        let _orderA = parseInt(
          this.shadowRoot
            .querySelector(`#${event.dataTransfer.getData(`text/plain`)}`)
            .parentNode.getAttribute(`data-order`)
        );
        let _orderB = parseInt(
        ...
        _orderArray.forEach(
          (_v, _index) =>
            (this.shadowRoot
              .querySelector(`#pluginsList`)
              .querySelector(`li[data-order="${_v}"]`).style.order = _index)
        );
